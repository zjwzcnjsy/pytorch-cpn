...
anno_root = '/home/jinsy/res/datasets/mscoco/annotations/'
...
def trans_anno(ori_file, target_file, is_val):
    ...
    if isfile(ori_anno) is False:
        no_ori = True
    if file_exist is False and no_ori is False:
        ...
        for img_id in tqdm(coco_ids):
            ...
            for ann in anns:
                ...
                unit = dict(num_keypoints=num_keypoints, keypoints=keypoints)
                x1, y1, width, height = bbox
                x2 = x1 + width
                y2 = y1 + height
                unit['GT_bbox'] = [int(x1), int(y1), int(x2), int(y2)]
                single_data['unit'] = unit
                imgInfo = dict(imgID=img_id, img_paths=file_name)
        ...
        print('done')
    if no_ori:
        print('''WARNING! There is no annotation file find at {}. Make sure you have put annotation files into the right folder.'''.format(ori_anno))
