...
class Config:
    ...
    root_dir = '/home/jinsy/res/datasets/mscoco'

    model = 'CPN50'  # option 'CPN50', 'CPN101'

    num_class = 17
    img_path = os.path.join(root_dir, 'val2017')
    ...
    use_GT_bbox = True
    if use_GT_bbox:
        gt_path = os.path.join(root_dir, 'annotations', 'COCO_2017_val.json')
    else:
        # if False, make sure you have downloaded the val_dets.json and place it into annotation folder
        gt_path = os.path.join(root_dir, 'annotations', 'val_dets.json')
    ori_gt_path = os.path.join(root_dir, 'annotations', 'person_keypoints_val2017.json')
